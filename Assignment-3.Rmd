...
SN refers to the reference sequence name. The SN tags and all individual AN names in all \@SQ lines must be distinct.

LN refers to the reference sequence length. Range: [1, 2\^31-1]
...
cat("Number of reads that align to chromosome X: ")
cat(nrow(X_reads))
```
...
cat("Mean Base Quality: ")
cat(mean_phred)
cat("Mean Percent error: ")
cat(10^(mean_phred / (-10)) *100)
cat("%")
```
...
quality_plot <- ggplot(long_quality_data, aes(x = factor(Position), y = QualityScore)) +
  geom_boxplot(outlier.shape = NA, coef=5, fill = "lightblue", color = "darkblue") +  # Extend whiskers to max range
...
cat("Number of leftmost mapping position within range: ")
cat(length(Hspa8))
```
...
Yes, this cell should emit the tdTomato flurorofore. This could be useful for tracking the genetically modified mouse line.
...
We are going to just choose the mutations that we would say are intronic
...
These genes could be of interest in study of disease if they have a high impact and our protein encoding variants

#### Q5.9. Consequences of short reads

-   If he reads are too short to span longer insertions (\>60bp), this could lead to incomplete or ambiguous alignments.

-   Mapping difficulties arise when the read doesn't align well to the reference due to the large insertion.

-   The absence of spanning reads reduces the accuracy and confidence in detecting longer indels so you would have to increase your read depth drastically to try and increase accuracy or mappability.
...
ad_values <- lapply(seq_along(allele_format), function(i) {
  # Find the index of "AD" in the labels list
  ad_index <- which(allele_format[[i]] == "AD")
  
  # Extract the metric value at that index from the corresponding metrics list
